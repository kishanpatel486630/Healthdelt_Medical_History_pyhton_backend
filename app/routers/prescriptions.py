...
def get_active_prescriptions(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    prescriptions = (
        ...
        .filter(Prescription.patientId == current_user.id, Prescription.status == "ACTIVE")
...
def get_prescription(prescription_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Prescription).filter(
        Prescription.id == prescription_id, Prescription.patientId == current_user.id
    ).first()
    if not p:
    ...
    return {
        ...
        "prescription": {
            ...
            "issuedDate": p.issuedDate.isoformat() if p.issuedDate else None,
            "medicines": [
                {"id": m.id, "name": m.name, "dosage": m.dosage, "frequency": m.frequency, "timing": m.timing, "duration": m.duration, "instructions": m.instructions}
                for m in p.medicines
            ],
        },
    }
...
def share_prescription(prescription_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    audit = AuditLog(userId=current_user.id, action="SHARE", resource="prescription", resourceId=p.id)
    db.add(audit)
    db.commit()
    return {"success": True}
